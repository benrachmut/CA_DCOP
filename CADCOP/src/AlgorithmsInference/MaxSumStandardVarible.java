...
public class MaxSumStandardVarible extends AgentVariableInference {
	...
	// OmerP - will loop over the neighbors and will send to each one of the neighbors. 
	@Override
	...
	protected void produceEmptyMessage() {
		...
		for (NodeId i : functionMsgs.keySet()) { // Start loop over the neighbors.
			...
			MsgAlgorithmFactor newMsg = new MsgAlgorithmFactor(this.getNodeId(), i, sentTable, 0, this.time); //Create new factor message.
	...
	//OmerP - chooses the minimal value in the table and subtract it from each value in the table with 
	...
	//OmerP - chooses the minimal value in the table and subtract it from each value in the table. 
	...
	protected double[] dampedMessage(double[] currentMessage, double[] lastMessage) {
		...
		if(dampingPrint) {printDampedTable(lastMessageAfterAlpha, currentMessageAfterAlpha, dampedMessage);}
	...
	protected void printSentdMessage(MsgAlgorithmFactor msg) {
		...
		System.out.println("Computation Counter:("+ this.computationCounter + "), VariableNode:(" + msg.getSenderId().getId1() + "," + msg.getSenderId().getId2() + ") SENT a message for FunctionNode ("
	...
	protected void printReceivedMessage(MsgAlgorithmFactor msg) {
		...
		System.out.println("Computation Counter:("+ this.computationCounter + "), VariableNode:(" + msg.getRecieverId().getId1() + "," + msg.getRecieverId().getId2() + ") RECEIVED a message from FunctionNode ("
	...
	protected void printValueAssignment(int valueAssignment, long[] belief) {
		...
		System.out.println("Computation Counter:("+ this.computationCounter + ") ,VariableNode:(" + this.getNodeId().getId1() + "," + this.getNodeId().getId2() + ") belief is " + Arrays.toString(belief) + ", and value assignment is:" + valueAssignment +".\n");
	...
	protected void printDampedTable(double[] oldTable, double[] newTable, double[] dampedTable) {
		...
		System.out.println("VariableNode:(" + this.getNodeId().getId1() + "," + this.getNodeId().getId2() + ") "
				+ "old table: " + Arrays.toString(oldTable) + " new table: " + Arrays.toString(newTable) + ", and damped table: " +  Arrays.toString(dampedTable) + ".\n");
	...
	protected void printFlag() {
		...
		if(this.receiveMessageFlag) {
			...
			System.out.println("VariableNode:(" + this.getNodeId().getId1() + "," + this.getNodeId().getId2() + "), Flag is DOWN.\n");
